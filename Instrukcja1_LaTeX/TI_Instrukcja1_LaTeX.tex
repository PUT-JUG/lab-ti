...
\begin{document}
	...
	Preambuła wstawiona przez edytor zawiera od razu kilka pakietów. Pierwszym z nich jest pakiet \verb|\usepackage[utf8]{inputenc}| informujący kompilator o kodowaniu pliku źródłowego. Po utworzeniu pliku nie należy zmieniać oznaczenia kodowania, gdyż uniemożliwi to kompilację dokumentu! 
	...
	Drugim pakietem wstawionym przez kreator jest \verb|\usepackage[T1]{fontenc}| informujący kompilator o kodowaniu czcionek w dokumencie wynikowym. Dla języka polskiego prawidłowym zestawem jest \emph{T1}. Brak tego pakietu lub ustawienie niewłaściwej opcji spowoduje błędy w kompilacji lub wstawienie niewłaściwych liter w miejsce polskich znaków diakrytyzowanych. Trzecim pakietem jest pakiet \verb|\usepackage[polish]{babel}|. Pakiet ten pozwala na dostosowanie między innymi podziału słów między wierszami według reguł języka polskiego. Oprócz powyższych warto dodać jeszcze trzeci pakiet: \verb|\usepackage{polski}|, dostosowujący pozostałe aspekty \LaTeX a.
	...
	\begin{itemize}
		\item \verb|\title{}| -- definiuje tytuł dokumentu,
		\item \verb|\author{}| -- definiuje autora/autorów,
		\item \verb|\date{}| -- zmienia datę pod tytułem.
	\end{itemize}
	...
	\begin{lstlisting}
		\maketitle
	\end{lstlisting}
	Tytuł pojawi się w dokumencie w miejscu wywołania tego polecenia. Komenda \verb|\date{}| różni się od pozostałych, gdyż nawet przy braku zdefiniowanej daty zostanie wstawiona data kompilacji. Polecenia tego używa się gdy konieczna jest zmiana daty.
	...
	\begin{itemize}
		\item \verb|\part{tytuł}| -- część,
		\item \verb|\chapter{tytuł}| -- rozdział, dostępny tylko w dokumentach typu książka (book) i raport (report),
		\item \verb|\section{tytuł}| -- sekcja,
		\item \verb|\subsection{tytuł}| -- podsekcja,
		\item \verb|\subsubsection{tytuł}| -- podpodsekcja,
		\item \verb|\paragraph{tytuł}| -- paragraf,
		\item \verb|\subparagraph{tytuł}| -- podparagraf.
	\end{itemize}
	...
	\LaTeX\ ignoruje białe znaki po komendach. Jeżeli potrzebna jest spacja po komendzie należy użyć polecenia wstawienia spacji: ,,\verb|\ |''.
	...
	W dokumentach pisanych w \LaTeX u nie jest dobrą praktyką, aby zmieniać lokalnie formatowanie lub rozmiar czcionek, gdyż prowadzi to do powstania niespójności stylistycznych. Zmian takich należy dokonywać na poziomie stylu całego dokumentu, na przykład poprzez zmianę rozmiaru czcionki w argumencie opcjonalnym polecenia \verb|\documentclass[arg. opcjonalne]{typ dokumentu}|.
	
	Czasem konieczne są jednak pewne zmiany, aby wyróżnić pewne fragmenty tekstu i \LaTeX\ udostępnia takie narzędzia:
	\begin{itemize}
		\item \verb|\emph{tekst}| -- wyróżnia tekst, sposób wyróżnienia zależny jest od stylu dokumentu,
		\item \verb|\textbf{tekst}| -- pogrubia czcionkę,
		\item \verb|\textit{tekst}| -- pochyla czcionkę,
		\item \verb|\underline{tekst}| -- podkreśla tekst,
	\end{itemize}

	Możliwe jest też lokalne zdefiniowanie rozmiaru tekstu:
	\begin{itemize}
		\item \verb|\Huge| -- {\Huge tekst},
		\item \verb|\huge| -- {\huge tekst},
		\item \verb|\LARGE| -- {\LARGE tekst},
		\item \verb|\Large| -- {\Large tekst},
		\item \verb|\large| -- {\large tekst},
		\item \verb|\normalsize| -- {\normalsize tekst},
		\item \verb|\small| -- {\small tekst},
		\item \verb|\footnotesize| -- {\footnotesize tekst},
		\item \verb|\scriptsize| -- {\scriptsize tekst},
		\item \verb|\tiny| -- {\tiny tekst},
	\end{itemize}
	...
	\begin{itemize}
		\item \verb|$\sin x$| -- pojedyncze dolary umieszczają formułę w linii wraz z tekstem: $\sin x$
		\item \verb|$$\sin x$$| -- podwójne dolary umieszczają formułę w osobnej linii: $$\sin x$$
		\item \verb|\begin{equation}\sin x\end{equation}| -- otoczenie \emph{equation} umieszcza formułę w osobnej linii i nadaje jej numer: \begin{equation}\sin x\end{equation}
	\end{itemize}
	...
	\begin{itemize}
		\item \verb|$x^{2b}$ | = $x^{2b}$,
		\item \verb|$x_{2b}$ | = $x_{2b}$,
		\item \verb|$\frac{a}{b}$ | = $\frac{a}{b}$,
		\item \verb|$\int i(t) dt$ | = $\int i(t) dt$,
		\item \verb|$\int\limits_{-T}^{T}i(t)dt$| = $\int\limits_{-T}^{T} i(t)dt$.
	\end{itemize}
	
	\zadanie{Odwzoruj pierwszy wzór matematyczny z dokumentu do odtworzenia. W jakim otoczeniu matematycznym został on zapisany?}
	
	W \LaTeX u nawiasy okrągłe i kwadratowe w formułach matematycznych można stosować wprost: $(),[]$. W przypadku nawiasów klamrowych należy poprzedzić je ukośnikami wstecznymi. Na przykład \verb|$\{ \}$|, daje: $\{\}$. Nawiasy takie jednak nie dostosują swoich rozmiarów do zawartości. Aby uzyskać taki efekt należy użyć poleceń \verb|\left| i \verb|\right|:
	
	 \verb|$\left(\frac{a}{b}\right)$| = $\left(\frac{a}{b}\right)$. 
	 
	 Polecenia te tworzą otoczenie nawiasowe, w związku z czym nie mogą się przecinać z innymi otoczeniami. Znaki nawiasów nie muszą sobie odpowiadać, można otworzyć otocznie nawiasem okrągłym i~zamknąć kwadratowym: \verb|$\left(\frac{a}{b}\right]$| = $\left(\frac{a}{b}\right]$. Jeżeli nie chcemy aby z którejś strony pojawił się znak nawiasu należy użyć kropki, samo otoczenie musi jednak zostać zamknięte: \verb|$\left\{\frac{a}{b}\right.$| = $\left\{\frac{a}{b}\right.$.
